...
const formSchema = z
  .object({
    ...
    availableToTime: z
      .string()
      .min(1, { message: "Hora de termino é obrigatória." }),
  })
  ...
  const upsertDoctorAction = useAction(upsertDoctor, {
    onSuccess: () => {
      const message = doctor
        ? `Médico ${doctor.name} atualizado com sucesso!`
        : "Médico adicionado com sucesso!";
      toast.success(message);
      onSuccess?.();
    },
    onError: () => {
      toast.error("Erro ao adicionar/atualizar médico.");
    },
  });
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  Dia <span className="underline">inicial</span> de
                  disponibilidade
                </FormLabel>
                <Select
                  onValueChange={field.onChange}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  Dia <span className="underline">final</span> de
                  disponibilidade
                </FormLabel>
                <Select
                  onValueChange={field.onChange}
